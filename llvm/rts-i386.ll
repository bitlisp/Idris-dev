...
declare void @__gmp_set_memory_functions(i8* (i32)*, i8* (i8*, i32, i32)*, void (i8*, i32)*)
declare void @__gmpz_init(%mpz*)
declare i32 @__gmpz_get_si(%mpz*)
declare double @__gmpz_get_d(%mpz*)
declare void @__gmpz_set(%mpz*, %mpz*)
declare void @__gmpz_init_set(%mpz*, %mpz*)
declare void @__gmpz_init_set_si(%mpz*, i32)
declare void @__gmpz_init_set_d(%mpz*, double)
...
declare i32 @__gmpz_cmp_si(%mpz*, i32) nounwind readonly
...
define internal i8* @gmp_realloc(i8* %ptr, i32 %old, i32 %new) {
  %1 = tail call i8* @GC_realloc(i8* %ptr, i32 %new)
  ret i8* %1
}

define internal void @gmp_free(i8* %ptr, i32 %size) {
...
define internal void @idris_memset(i8* %ptr, i32 %off, i8 %val, i32 %len) {
  ...
  tail call void @llvm.memset.p0i8.i32(i8* %3, i8 %val, i32 %len, i32 0, i1 false)
  ret void
}
...
define internal void @idris_memmove(i8* %dest, i8* %src, i32 %doff, i32 %soff, i32 %len) {
  ...
  %2 = add i32 %1, %doff
  %3 = inttoptr i32 %2 to i8*
  %4 = ptrtoint i8* %src to i32
  %5 = add i32 %4, %soff
  %6 = inttoptr i32 %5 to i8*
  tail call void @llvm.memmove.p0i8.p0i8.i32(i8* %3, i8* %6, i32 %len, i32 0, i1 false)
  ret void
}
...
define internal void @idris_poke(i8* %ptr, i32 %off, i8 %val) {
  ...
  %2 = add i32 %1, %off
  %3 = inttoptr i32 %2 to i8*
  store i8 %val, i8* %3, align 1
  ret void
}

define internal fastcc i8* @readStr(i8* nocapture %h) nounwind {
  %1 = tail call noalias i8* @GC_malloc_atomic(i32 256) nounwind
  %2 = getelementptr inbounds i8* %1, i32 255
...
.preheader:                                       ; preds = %0, %11
  %indvars.iv = phi i32 [ %indvars.iv.next, %11 ], [ 256, %0 ]
...
; <label>:7                                       ; preds = %.preheader
  %8 = getelementptr inbounds i8* %line_ptr.0, i32 -1
  %9 = load i8* %8, align 1
  %10 = icmp eq i8 %9, 10
  br i1 %10, label %.critedge, label %11

; <label>:11                                      ; preds = %7
  %indvars.iv.next = add i32 %indvars.iv, 256
  %12 = tail call i8* @GC_realloc(i8* %line_buf.0, i32 %indvars.iv.next) nounwind
  %.sum18 = or i32 %indvars.iv, 255
  %13 = getelementptr inbounds i8* %12, i32 %.sum18
  store i8 1, i8* %13, align 1
  %.sum17 = add i32 %indvars.iv, -1
  %14 = getelementptr inbounds i8* %12, i32 %.sum17
...
.critedge:                                        ; preds = %.preheader, %7, %11, %0
  %.0 = phi i8* [ getelementptr inbounds ([1 x i8]* @emptyStr, i32 0, i32 0), %0 ], [ %line_buf.0, %.preheader ], [ %line_buf.0, %7 ], [ getelementptr inbounds ([1 x i8]* @emptyStr, i32 0, i32 0), %11 ]
  ret i8* %.0
}

define void @putStr(i8* %str) nounwind {
  %1 = tail call i32 (i8*, ...)* @printf(i8* getelementptr inbounds ([3 x i8]* @strfmt, i32 0, i32 0), i8* %str) nounwind
  ret void
}

define internal fastcc i8* @strConcat(i8* %l, i8* %r) nounwind {
  %1 = call i32 @strlen(i8* %l) nounwind readonly
  %2 = call i32 @strlen(i8* %r) nounwind readonly
  %3 = add i32 %1, 1
  %4 = add i32 %3, %2
  %5 = call noalias i8* @GC_malloc_atomic(i32 %4) nounwind
...
define internal fastcc noalias i8* @intStr(i64 %i) nounwind {
  %1 = tail call noalias i8* @GC_malloc_atomic(i32 12) nounwind
  %2 = tail call i32 (i8*, i32, i8*, ...)* @snprintf(i8* %1, i32 12, i8* getelementptr inbounds ([4 x i8]* @intfmt, i64 0, i64 0), i64 %i) nounwind
  ret i8* %1
}
...
define internal fastcc i8* @strCons(i32 %c, i8* %s) nounwind {
  %1 = call i32 @strlen(i8* %s) nounwind readonly
  %2 = add i32 %1, 2
  %3 = call noalias i8* @GC_malloc_atomic(i32 %2) nounwind
  %4 = trunc i32 %c to i8
  store i8 %4, i8* %3
  %5 = getelementptr inbounds i8* %3, i32 1
  call void @llvm.memcpy.p0i8.p0i8.i32(i8* %5, i8* %s, i32 %1, i32 1, i1 false)
  %6 = add i32 %1, 1
  %7 = getelementptr inbounds i8* %3, i32 %6
  store i8 0, i8* %7
  ret i8* %3
}
